...
/obj/item/weapon/gun/projectile/automatic/mg34
	...
	firemodes = list(
        list(name="short bursts", burst=5, move_delay=10, dispersion = list(0.3, 0.6, 0.6, 0.8, 0.9), burst_delay = 1.0, recoil = 1.4),
        list(name="long bursts", burst=10, move_delay=12, dispersion = list(0.5, 0.7, 0.7, 0.8, 1.0), burst_delay = 1.0, recoil = 2.8)
		)
...
/obj/item/weapon/gun/projectile/automatic/mg34/mg42
	name = "MG-42"
	desc = "German machinegun chambered in 7.92x57mm Mauser. 'Hitler's Buzzsaw' is the name for the toy. Make sure to bring a LOT of belts"
	icon_state = "mg42closed200"
	item_state = "mg42closedmag"
	w_class = 5
	heavy = TRUE
	max_shells = 200
	caliber = "a792x57"
	...
	reload_sound 	= 'sound/weapons/guns/interact/lmg_magin.ogg'
	cocked_sound 	= 'sound/weapons/guns/interact/lmg_cock.ogg'
	fire_sound = 'sound/weapons/guns/fire/mg42_fire.ogg'
	requires_two_hands = FALSE
	wielded_icon = "assault-wielded"

	firemodes = list(
        list(name="short bursts", burst=8, move_delay=10, dispersion = list(0.6, 0.7, 0.8, 1.0, 1.2), burst_delay = 0.9, recoil = 1.9),
        list(name="long bursts", burst=16, move_delay=15, dispersion = list(0.6, 0.8, 0.8, 0.9, 1.2), burst_delay = 0.9, recoil = 3.9)
		)
...
/obj/item/weapon/gun/projectile/automatic/mg34/mg42/special_check(mob/user)
	if (cover_open)
		user << "<span class='warning'>[src]'s cover is open! Close it before firing!</span>"
		return FALSE
	return ..()

/obj/item/weapon/gun/projectile/automatic/mg34/mg42/toggle_cover(mob/user)
	cover_open = !cover_open
	user << "<span class='notice'>You [cover_open ? "open" : "close"] [src]'s cover.</span>"
	update_icon()

/obj/item/weapon/gun/projectile/automatic/mg34/mg42/attack_self(mob/user as mob)
	if (cover_open)
		toggle_cover(user) //close the cover
		playsound(loc, 'sound/weapons/guns/interact/lmg_close.ogg', 100, TRUE)
	else
...
/obj/item/weapon/gun/projectile/automatic/mg34/mg42/attack_hand(mob/user as mob)
	if (!cover_open && user.get_inactive_hand() == src)
		toggle_cover(user) //open the cover
		playsound(loc, 'sound/weapons/guns/interact/lmg_open.ogg', 100, TRUE)
	else
...
/obj/item/weapon/gun/projectile/automatic/mg34/mg42/update_icon()
	icon_state = "mg42[cover_open ? "open" : "closed"][ammo_magazine ? round(ammo_magazine.stored_ammo.len, 10) : "-empty"]"

/obj/item/weapon/gun/projectile/automatic/mg34/mg42/load_ammo(var/obj/item/A, mob/user)
	if (!cover_open)
		user << "<span class='warning'>You need to open the cover to load [src].</span>"
		return
	..()

/obj/item/weapon/gun/projectile/automatic/mg34/mg42/unload_ammo(mob/user, var/allow_dump=1)
	if (!cover_open)
		user << "<span class='warning'>You need to open the cover to unload [src].</span>"
		return
	..()
